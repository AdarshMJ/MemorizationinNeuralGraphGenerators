...
class FeatureDecoder(nn.Module):
    def __init__(self, latent_dim: int, hidden_dim: int, feature_dim: int, n_nodes: int, mp_layers: int = 2):
        super().__init__()
        self.n_nodes = n_nodes
        self.hidden_dim = hidden_dim
        self.init_proj = nn.Linear(latent_dim, n_nodes * hidden_dim)
        self.layers = nn.ModuleList([nn.Linear(hidden_dim * 2, hidden_dim) for _ in range(mp_layers)])
        self.norms = nn.ModuleList([nn.LayerNorm(hidden_dim) for _ in range(mp_layers)])
        self.out_proj = nn.Linear(hidden_dim, feature_dim)
        self.activation = nn.ReLU()
        self.register_buffer("eye", torch.eye(n_nodes))

    def forward(self, z_feat: torch.Tensor, adj: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        batch_size = z_feat.size(0)
        h = self.init_proj(z_feat).view(batch_size, self.n_nodes, self.hidden_dim)
        h = self.activation(h)

        adj_norm = self._normalize_adj(adj)
        for layer, norm in zip(self.layers, self.norms):
            agg = torch.bmm(adj_norm, h)
            h = torch.cat([h, agg], dim=-1)
            h = layer(h)
            h = self.activation(norm(h))
    ...
    def _normalize_adj(self, adj: torch.Tensor) -> torch.Tensor:
        ...
        eye = self.eye.unsqueeze(0).to(adj.device)
        adj = adj + eye
...
class JointVariationalAutoEncoder(nn.Module):
    def __init__(self,
        ...
        self.feat_proj = nn.Linear(latent_dim, latent_dim)

        self.struct_decoder = Decoder(latent_dim, hidden_dim_struct_dec, n_layers_struct_dec, n_max_nodes)
        self.feature_decoder = FeatureDecoder(latent_dim, hidden_dim_feat_dec, input_dim, n_max_nodes)
        self.label_decoder = LabelDecoder(hidden_dim_feat_dec, num_classes)

    def encode_stats(self, data: Data) -> Tuple[torch.Tensor, torch.Tensor]:
        h = self.encoder(data)
        mu = self.fc_mu(h)
    ...
    def decode(self, z: torch.Tensor, adj_override: torch.Tensor = None) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        z_struct = self.struct_proj(z)
        z_feat = self.feat_proj(z)

        adj_pred = self.struct_decoder(z_struct)
        adj_for_features = adj_override if adj_override is not None else adj_pred

        node_repr, features = self.feature_decoder(z_feat, adj_for_features)
        label_logits = self.label_decoder(node_repr)
        return adj_pred, features, label_logits
    ...
    def loss_function(self, data: Data) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        mu, logvar = self.encode_stats(data)
        z = self.reparameterize(mu, logvar)
        adj_target = data.A
        adj_override = adj_target if (self.training and self.use_teacher_forcing) else None
        adj_pred, feat_pred, label_logits = self.decode(z, adj_override=adj_override)
        node_mask = data.node_mask.unsqueeze(-1).float()
        feat_target = data.x_padded
        label_mask = data.node_mask
        label_target = data.y_padded

        adj_loss = F.l1_loss(adj_pred, adj_target)
        denom = node_mask.sum().clamp(min=1.0)
        feat_loss = ((feat_pred - feat_target) ** 2 * node_mask).sum() / denom
        ...
        kld = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp()) / adj_pred.size(0)

        total_loss = adj_loss + self.lambda_feat * feat_loss + self.lambda_label * label_loss + self.beta * kld
        logs = {
    ...
    def reconstruct(self, data: Data) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        self.eval()
        with torch.no_grad():
            mu, _ = self.encode_stats(data)
            adj, features, label_logits = self.decode(mu)
        return adj, features, label_logits
...
def train_joint(model: JointVariationalAutoEncoder,
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(
            f"Epoch {epoch:03d} | Train Loss: {avg_train['total']:.4f} | Val Loss: {avg_val['total']:.4f} "
            f"| Adj: {avg_val['adj']:.4f} | Feat: {avg_val['feat']:.4f} | Label: {avg_val['label']:.4f}"
        )
