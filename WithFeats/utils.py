...
def construct_nx_from_adj(adj, node_features=None, node_labels=None):
    ...
    if node_features is not None:
        if isinstance(node_features, torch.Tensor):
            ...
            features_np = np.asarray(node_features)
        for node in G.nodes():
            if node < features_np.shape[0]:
                G.nodes[node]['feature_vector'] = features_np[node].tolist()
    ...
    if node_labels is not None:
        ...
        for node in G.nodes():
            if node < labels_np.shape[0]:
                G.nodes[node]['class_label'] = int(labels_np[node])
...
def eval_autoencoder(test_loader, autoencoder, n_max_nodes, device):
    ...
    try:
        for data in test_loader:
            ...
            # Get node labels from data if available
            if hasattr(data, 'y') and data.y is not None:
                node_labels = data.y.detach().cpu().numpy()
            else:
                node_labels = None

            for i in range(data.A.size(0)):
                G_true = construct_nx_from_adj(data.A[i,:,:].detach().cpu().numpy())
                G_rec = construct_nx_from_adj(adj_rec[i,:,:].detach().cpu().numpy())
                
                # Assign actual node labels if available
                if node_labels is not None:
                    # For batched data, get labels for this graph
                    start_idx = i * n_max_nodes
                    end_idx = start_idx + n_max_nodes
                    labels_this_graph = node_labels[start_idx:end_idx]
                    
                    # Assign labels to nodes in ground truth graph
                    for node_idx, node in enumerate(G_true.nodes()):
                        if node_idx < len(labels_this_graph):
                            G_true.nodes[node]['label'] = int(labels_this_graph[node_idx])
                    
                    # Assign labels to nodes in reconstructed graph (same labels)
                    for node_idx, node in enumerate(G_rec.nodes()):
                        if node_idx < len(labels_this_graph):
                            G_rec.nodes[node]['label'] = int(labels_this_graph[node_idx])
                else:
                    # Fallback: use dummy label
                    for node in G_true.nodes():
                        G_true.nodes[node]['label'] = 1
                    for node in G_rec.nodes():
                        G_rec.nodes[node]['label'] = 1
        ...
        print(f'Note: Graph kernel evaluation skipped due to: {e}')
        print('This does not affect training. Using simple reconstruction loss instead.')
        
        # Simple alternative: compute reconstruction accuracy
        with torch.no_grad():
            total_correct = 0
            total_edges = 0
            for data in test_loader:
                data = data.to(device)
                adj_rec = autoencoder(data)
                adj_rec_binary = (adj_rec > 0.5).float()
                correct = (adj_rec_binary == data.A).float().sum()
                total = data.A.numel()
...
def calculate_stats_graph(G):
    ...
    if num_nodes == 0 or num_edges == 0:
        # Return a padded vector of zeros (same length as expected statistics)
        stats.extend([0.0] * 13)
        return stats
