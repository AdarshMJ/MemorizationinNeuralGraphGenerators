...
parser.add_argument('--n-properties', type=int, default=18)
...
if args.use_synthetic_data:
    ...
    # Load actual homophily measurements if available
    homophily_log = None
    log_path = Path(args.synthetic_data_path).with_name(Path(args.synthetic_data_path).name.replace('_graphs.pkl', '_log.csv'))
    if log_path.exists():
        try:
            homophily_log = pd.read_csv(log_path)
            homophily_log = homophily_log.set_index('graph_idx')
            print(f"Loaded homophily log from {log_path}")
        except Exception as exc:
            print(f"Warning: failed to load homophily log ({exc}). Proceeding without refresh.")

    processed_data_lst = []
    for idx, graph in enumerate(tqdm(data_lst, desc="Processing synthetic graphs")):
        ...
        if hasattr(graph, 'stats'):
            ...
            if graph.stats.shape[-1] < args.n_properties:
                pad = args.n_properties - graph.stats.shape[-1]
                graph.stats = F.pad(graph.stats, (0, pad))
            elif graph.stats.shape[-1] > args.n_properties:
        ...
        # Refresh homophily stats from log if available
        if homophily_log is not None:
            if hasattr(graph, 'graph_idx'):
                log_idx = int(graph.graph_idx)
            else:
                log_idx = idx
                graph.graph_idx = log_idx
            if log_idx in homophily_log.index:
                row = homophily_log.loc[log_idx]
                if hasattr(graph, 'feature_homophily'):
                    graph.feature_homophily = torch.tensor(float(row['actual_feature_hom']))
                if hasattr(graph, 'stats') and graph.stats.shape[-1] >= 3:
                    # Overwrite last three entries with measured homophily to ensure accuracy
                    graph.stats[0, -3:] = torch.tensor([
