...
# Configuration
N_VALUES = [10, 20, 50, 100, 500, 1000, 2500]  # Training set sizes to test (avoiding extreme N=1)
TEST_SET_SIZE = 500  # Number of conditioning graphs held out from both models
...
LEARNING_RATE = 0.001  # Same learning rate for all experiments
...
N_PROPERTIES = 15  # Same number of conditioning properties for all experiments
...
def compute_wl_similarity(G1, G2):
    ...
    don't have semantic node labels. This makes WL kernel focus on structural
    patterns rather than node attributes.
    """
    ...
    try:
        # Assign node labels based on degree (topology-derived)
        # This captures structural roles of nodes
        for node in G1.nodes():
            G1.nodes[node]['label'] = G1.degree(node)
        for node in G2.nodes():
            G2.nodes[node]['label'] = G2.degree(node)
...
def run_experiment_for_N(N, split_config, output_dir):
    ...
    for data in S1:
        ...
        S1_training_graphs_nx.append(construct_nx_from_adj(adj))
...
def visualize_single_experiment(results):
    ...
    if example_G1 is not None and example_G2 is not None:
        ...
        for ax, G in zip(axes, [example_G1, example_G2]):
